...
try:
    model = joblib.load("C:\\Users\\k bhageeratha reddy\\Downloads\\stroke-ai-website\\models\\best_model.pkl")
    scaler = joblib.load("C:\\Users\\k bhageeratha reddy\\Downloads\\stroke-ai-website\\models\\scaler.pkl")
    feature_names = joblib.load("C:\\Users\\k bhageeratha reddy\\Downloads\\stroke-ai-website\\models\\columns.pkl")
    needs_scaling = joblib.load("C:\\Users\\k bhageeratha reddy\\Downloads\\stroke-ai-website\\models\\needs_scaling.pkl")
    
    # Load training data for SHAP background
    clean_data = pd.read_csv("C:\\Users\\k bhageeratha reddy\\Downloads\\stroke-ai-website\\data\\final_heart_clean.csv")
...
async def explain_lime(patient: PatientInput):
    try:
        ...
        # Scale if needed (IMPORTANT: use float64)
        if needs_scaling:
            patient_processed = scaler.transform(patient_df).astype(np.float64)
            training_data = scaler.transform(X_background).astype(np.float64)
        else:
            patient_processed = patient_df.values.astype(np.float64)
            training_data = X_background.values.astype(np.float64)

        # ✅ Wrap model (CRITICAL FIX)
        def predict_fn(x):
        ...
        # ✅ Generate explanation (NO top_labels)
        ...
        # ✅ Plot safely (no label hardcoding)
        fig = exp.as_pyplot_figure()
        plt.title("LIME Explanation - Top Features", fontsize=16, fontweight='bold')
        plt.tight_layout()
        ...
        return {
            ...
            "feature_weights": [
                ...
                for feat, weight in lime_values
            ]
        }

    except Exception as e:
        import traceback
        logger.error("LIME ERROR:\n" + traceback.format_exc())
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/explain/lime/html")
async def explain_lime_html(patient: PatientInput):
    try:
        from lime.lime_tabular import LimeTabularExplainer
        import traceback
